    ...
    if(this.ball.y + this.ball.dy > this.canvas.height - this.ball.radius - this.paddle.height) {
      if(this.ball.collidedWithPaddle(this.paddle)) {
        ...
        if(this.ball.x < paddleMiddle) {
          this.ball.changeVelocity(-(Math.abs(this.ball.dx) * (0.5 + 2*(paddleMiddle - this.ball.x)/halfPaddle)));
        } else {
          this.ball.changeVelocity(Math.abs(this.ball.dx) * (0.5 + 2*(this.ball.x - paddleMiddle)/halfPaddle));
        }
